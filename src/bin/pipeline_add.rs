...
impl PipelinedAdder {
    ...
    fn tick(&mut self, input_a: u32, input_b: u32) {
...
fn main() {
    ...
    adder.tick(10, 20);
    println!("Cycle 1 Output: {}", adder.get_output()); // Output: 0 (Pipeline is filling)

    // Cycle 2: Feed in 5 and 5
    adder.tick(5, 5);
    println!("Cycle 2 Output: {}", adder.get_output()); // Output: 30 (10 + 20)

    // Cycle 3: Feed in 0 and 0
    adder.tick(0, 0);
    println!("Cycle 3 Output: {}", adder.get_output()); // Output: 10 (5 + 5)
}
